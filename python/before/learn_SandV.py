...
# S and V(Salinity causes velocity) and VS(Velocity causes Salinity)
...
def main():
    ...
    data = numpy.zeros((rawdata.shape[0] * 2, rawdata.shape[2] * 2),
                       dtype=numpy.float32)
    sv = numpy.zeros((rawdata.shape[0], rawdata.shape[2] * 2), dtype=numpy.float32)
    vs = numpy.zeros((rawdata.shape[0], rawdata.shape[2] * 2), dtype=numpy.float32)

    for i in range(rawdata.shape[0]):
        sv[i, :rawdata.shape[2]] = rawdata[i, 0]      #   S(salinity)
        sv[i, rawdata.shape[2]:] = rawdata[i, 2]      #   V(Velocity)
        vs[i, :rawdata.shape[2]] = rawdata[i, 2]      #   V(Velocity)
        vs[i, rawdata.shape[2]:] = rawdata[i, 0]      #   S(water temperature)
